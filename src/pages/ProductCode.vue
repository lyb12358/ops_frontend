<template>
  <q-page class="q-pa-sm">
              ...
              <div class="text-left">
                <q-btn-group>
                  ...
                  <q-btn
                    ...
                    @click="openMultiImageDialog(props.row.id)"
                  />
                  <q-btn
                    dense
                    label="查看细节图"
                    icon="mdi-image-search"
                    @click="openImageCarousel(props.row.id)"
                  />
    ...
    <q-dialog v-model="multiImageDialogOpened" persistent>
      <q-card>
        <q-card-section class="row items-center">
          <div class="text-h6">上传细节图</div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
        <q-card-section>
          <q-uploader
            ...
            :url="api + multiImageUploadUrl"
            accept=".jpg, .jpeg, .png"
            multiple
            :headers="[{ name: 'id', value: this.expandId }]"
            style="max-width: 300px"
            @uploaded="multiImageUploaded"
            @failed="imageUploadFail"
            @added="addImageFile"
            field-name="file"
            name="file"
    ...
    <q-dialog v-model="imageCarouselOpened">
      <q-carousel
        animated
        v-model="slide"
        infinite
...
export default {
  data() {
    return {
      steps: [
        {
          target: '#xx1', // We're using document.querySelector() under the hood
          content: `这里是新增`
        },
        {
          target: '#xx2',
      ...
      multiImageDialogOpened: false,
      multiImageUploadUrl: '/imageUpload/multi/product',
      imageList: [],
      imageCarouselOpened: false,
      imageId: '',
      slide: 0
      //driver
    }
  ...
  methods: {
    ...
    thumbnailCheck(id, styleId, codeThumbnail, styleThumbnail) {
      if (!(codeThumbnail === null) && !(codeThumbnail === '')) {
        return this.api + '/image/' + id + '/' + codeThumbnail
      } else if (!(styleThumbnail === null) && !(styleThumbnail === '')) {
        return this.api + '/image/' + styleId + '/' + styleThumbnail
      } else {
    ...
    //multi image
    openMultiImageDialog(id) {
      this.expandId = id
      this.multiImageDialogOpened = true
    },
    // when image has just bean uploaded
    multiImageUploaded(info) {
      let response = JSON.parse(info.xhr.response)
      if (response.code == 20000) {
        this.notify('positive', response.msg)
        this.$refs.multiImageUpload.reset()
        this.multiImageDialogOpened = false
        this.request({
          pagination: this.serverPagination
        })
      } else {
    ...
    openImageCarousel(id) {
      this.imageId = id
      getProductMultiImageById(id)
        .then((response) => {
          this.imageList = response.data.data.split(';')
        })
        .catch((error) => {})
      this.$nextTick(() => {
        this.imageCarouselOpened = true
      })
    },
    multiImageCheck(item) {
      return this.api + '/image/multi/' + this.imageId + '/' + item
    },
